...
public class Enumeration<A> {
    ...
    private static <A> A index(LazyList<Finite<A>> ps, BigInteger i) {
        ...
        /*
         * if (ps.isEmpty()) { throw new RuntimeException("index " + i + " out
         * of range"); }
         *
         * Finite<A> head = ps.head(); BigInteger card = head.getCard(); if
         * (i.compareTo(card)<0) { return head.get(i); } return index(ps.tail(),
         * i.subtract(card));
         */
    }
