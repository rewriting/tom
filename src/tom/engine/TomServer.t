...
public class TomServer implements TomPluginOptions
{
  ...
  public OptionList requiredOptions()
  {
    ...
    OptionList prerequisites = `emptyOptionList();
        ...
        %match(Option h)
          {
            OptionBoolean[name="debug", valueB=True()] -> 
              { 
                prerequisites = `concOption(OptionBoolean("jCode", "", "", True()), prerequisites*);
            ...
            OptionString[name="destdir", valueS=v] -> 
              { 
                if(!v.equals(".")) // destdir is not set at its default value -> it has been changed
                  prerequisites = `concOption(OptionString("output", "", "", "", ""), prerequisites*);
            ...
            OptionString[name="output", valueS=v] -> 
              { 
                if(!v.equals("")) // output is not set at its default value -> it has been changed
                  prerequisites = `concOption(OptionString("destdir", "", "", ".", ""), prerequisites*);
  ...
  private void extractOptionBoolean(TNode optionBooleanNode)
  {
    ...
    %match(String valueB)
	    {
        ('true') ->
          { myOptions = `concOption(myOptions*, OptionBoolean(name, altName, description, True())); }
        ('false') ->
          { myOptions = `concOption(myOptions*, OptionBoolean(name, altName, description, False())); }
	    }
  ...
  ///////////// THE FOLLOWING METHODS ARE COPIED FROM TOM.JAVA /////////////
