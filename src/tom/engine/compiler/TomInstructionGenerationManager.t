	...
	public Instruction performGenerations(Constraint constraint){
		...
		mainLoop: while(true){		
			for(String i:generatorsNames){
				...
				TomIBaseGenerator prop = (TomIBaseGenerator)Class.forName(generatorsPackage + i);
				Instruction result = prop.generate(instruction);
				...
				// it's time to stop
				if (propCounter == propagatorsName.length) { break mainLoop; }
				// reinitialize
				intruction = result; 
			}
		} // end while
