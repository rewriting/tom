...
public class ASTFactory implements TomParserConstants {
  ...
  private String tokenListToString(List tokenList) {
    String s = "";
  ...
  private TomTerm tokenListToTerm(List tokenList) {
    TomTerm term = null;
    ...
    if(tokenList.size() == 1 && token.kind == TOM_IDENTIFIER) {
      return tsf().makeTomTerm_DollarAppl(makeOption(),
                                          tsf().makeTomName_Name(token.image),
                                          tsf().makeTomList());
    } else if(tokenList.size() == 2
              && token.kind == TOM_IDENTIFIER
              && ((Token) tokenList.get(1)).kind == TOM_STAR) {
      return makeVariableStar(makeOption(),token.image,"unknown type");
    } else {
      String name = tokenListToString(tokenList);
      return tsf().makeTomTerm_TargetLanguageToTomTerm(tsf().makeTargetLanguage_ITL(name));
    }
  }

  
  public TomTerm buildBackQuoteTerm(ArrayList tokenList) {
    Token token = null;
    Token previous = null;
    LinkedList accu = new LinkedList();
    int activeParenInAccu = 0;
    String name;
    List args;
    ...
    TomTerm term;
    Option option = makeOption();
    ListIterator lit = tokenList.listIterator();

    stackList.push(new LinkedList());

    while(lit.hasNext()) {
      previous = token;
      token = (Token)lit.next();
        //System.out.println("current token: " + token);
      switch(token.kind) {
          case TOM_LPAREN:
            if(previous.kind == TOM_IDENTIFIER) {
                // Case: id(
              name = tokenListToString(accu);
              accu.clear();
                //System.out.println("pushName: " + name); 
              args = new LinkedList();
              stackName.push(name);
              stackList.push(args);
            } else {
                // Case: (
                //System.out.println("addAccu: " + token.image); 
              accu.addLast(token);
              activeParenInAccu++;
            }
            break;
          case TOM_COMMA:
            term = tokenListToTerm(accu);
            if(term != null) {
              ((List)stackList.peek()).add(term);
            }
            accu.clear();
            break;
          case TOM_RPAREN:
            if(activeParenInAccu > 0) {
                // Case: (...)
                //System.out.println("addAccu: " + token.image); 
              accu.addLast(token);
              activeParenInAccu--;
            } else {
                // Case: flush the accu before the ')'
              term = tokenListToTerm(accu);
              if(term != null) {
                ((List)stackList.peek()).add(term);
              }
              ...
              name = (String) stackName.pop();
              args = (List) stackList.pop();

                //System.out.println("popName: " + name); 
                // Case: xxx.id()
              if(name.charAt(0) == '.') {
                name = name.substring(1);

                if(args.isEmpty()) {
                  option = makeOption(tsf().makeOption_Constructor(tsf().makeTomName_Name(name)));
                } else {
                  option = makeOption();
                }

                term = tsf().makeTomTerm_DollarAppl(option,
                                                    tsf().makeTomName_Name(name),
                                                    makeList(args));
                TomTerm previousTerm = (TomTerm) ((LinkedList)stackList.peek()).removeLast();
                term = tsf().makeTomTerm_DotTerm(previousTerm,term);
    ...
    List list = (List) stackList.pop();
    if(list.size() != 1) {
      System.out.println("buildBackQuoteTerm: list.size = " + list.size() + " is not correct");
      return null;
    } else {
      term = (TomTerm) list.get(0);

      System.out.println("dollarTerm = " + term);
