...
public class ModularNewKernelTyper {
  %include { ../../library/mapping/java/sl.tom}
  %include { ../adt/tomsignature/TomSignature.tom }

  %typeterm ModularNewKernelTyper {
    implement { ModularNewKernelTyper }
    is_sort(t) { ($t instanceof ModularNewKernelTyper) }
  }

  private SymbolTable symbolTable;
  private int freshNameCounter = 0;
  // Set of pairs (freshName,originalName)
  private HashMap<TomName,TomName> nameSubstitutions;
  ...
  private HashMap<TomType,TomType> typeSubstitutions;

  public ModularNewKernelTyper() {
    super();
  }
  ...
  protected TomName getFreshNameVar() {
    return `Name("f_r_e_s_h_n_a_m_e" + freshNameCounter++);
  }
   ...
   * <code>typeConstraints</code> and <code>typeSubstitutions</code>
   */
  private void init() {
    nameSubstitutions = new HashMap<TomName,TomName>();
    typeConstraints = `concTypeConstraint();
    typeSubstitutions = new HashMap<TomType,TomType>();
  }
   ...
   *        typeSubstitutions for each type variable)
    ...
    %match(code) {
      Tom(codes@concCode(_,_*)) -> {
        ...
        for(Code headCodeList : `codes.getCollectionconcCode()) {
          %match(headCodeList) {
            InstructionToCode(instruction@Match(CIList,options)) -> {
              try {
                Instruction IClone =
                  `BottomUp(AlphaConversion(this)).visitLight(`instruction);
                %match(IClone) {
                  Match(CILClone,_) -> {
                    inferConstraintInstructionList(`CILClone);
                    ...
                    // Replace in CIList which did not have its variables renamed
                    ConstraintInstructionList result =
                    replaceInConstraintInstructionList(`CIList);
  ...
  private void inferConstraintInstructionList(ConstraintInstructionList ciList) {
    ...
    %match(ciList) {
      concConstraintInstruction(headCIList@ConstraintInstruction(constraint,action,_),tailCIList*) -> {
        ...
        // TODO : Treat linearity here : add type constraints TypedVar(x,y)
   ...
   * </ul>
   */
  %strategy AlphaConversion(mnkt:ModularNewKernelTyper) extends Identity() {
    visit Instruction {
      Match(CIList@concConstraintInstruction(_,_*),option) -> {
        ConstraintInstructionList renamedCIList = `concConstraintInstruction();
        for (ConstraintInstruction cInstruction :
          `CIList.getCollectionconcConstraintInstruction()) {
          renamedCIList = `concConstraintInstruction(renamedCIList*,mnkt.RenameAndReplace(cInstruction));
        }
          return `Match(renamedCIList,option);
      }
    }
  }

  private ConstraintInstruction RenameAndReplace(ConstraintInstruction cInstruction) {
    try {
      nameSubstitutions.clear();
      `TopDown(FindVar(this)).visitLight(cInstruction);
      return `TopDown(Replace(this)).visitLight(cInstruction);
    
    } catch (tom.library.sl.VisitFailure e) {
      throw new TomRuntimeException("RenameAndReplace: failure on " +
          `cInstruction);
    }
  }

  %strategy FindVar(mnkt:ModularNewKernelTyper) extends Identity() {
    visit TomTerm {
      var -> {
        %match {
          (Variable|VariableStar)[AstName=name] << var -> {
            TomName newTName = mnkt.getFreshNameVar();
            if (!mnkt.nameSubstitutions.containsKey(`name)) {
              mnkt.nameSubstitutions.put(`name,newTName);
            }
          }
        }
      }
  ...
  %strategy Replace(mnkt:ModularNewKernelTyper) extends Identity() {
    visit TomTerm {
      var@Variable[Option=option,AstName=name,AstType=type,Constraints=constraints] -> {
        if (mnkt.nameSubstitutions.containsKey(`name)) {
          return `Variable(option,mnkt.nameSubstitutions.get(name),type,constraints);
        } else {
          return `var;  
        }
      }

      var@VariableStar[Option=option,AstName=name,AstType=type,Constraints=constraints] -> {
        if (mnkt.nameSubstitutions.containsKey(`name)) {
          return `VariableStar(option,mnkt.nameSubstitutions.get(name),type,constraints);
        } else {
          return `var;  
        }
      }
    ...
    visit BQTerm {
      bqvar@BQVariable[Option=option,AstName=name,AstType=type] -> {
        if (mnkt.nameSubstitutions.containsKey(`name)) {
          return `BQVariable(option,mnkt.nameSubstitutions.get(name),type);
        } else {
          return `bqvar;  
        }
      }

      bqvar@BQVariableStar[Option=option,AstName=name,AstType=type] -> {
        if (mnkt.nameSubstitutions.containsKey(`name)) {
          return `BQVariableStar(option,mnkt.nameSubstitutions.get(name),type);
        } else {
          return `bqvar;  
        }
      }
  ...
  %strategy solveConstraints(mnkt:ModularNewKernelTyper) extends Identity() {
    visit TypeConstraintList {
      ...
      concTypeConstraint(leftTCList*,Equation(typeVar@Type(_,TypeVar(_)),type@!typeVar),rightTCList*) -> {
        mnkt.typeSubstitutions.put(`typeVar,`type);
        %match {
          !concTypeConstraint() << leftTCList -> {
            if(mnkt.findTypeVars(`typeVar,`leftTCList)) {
              `leftTCList = mnkt.applyTypeSubstitution(`typeVar,`type,`leftTCList);
            }
          }

          !concTypeConstraint() << rightTCList -> {
            if(mnkt.findTypeVars(`typeVar,`rightTCList)) {
              `rightTCList = mnkt.applyTypeSubstitution(`typeVar,`type,`rightTCList);
            }
          }
        }
  ...
  private TypeConstraintList applyTypeSubstitution(TomType oldtt, TomType newtt,
    ...
    try {
      ...
      throw new RuntimeException("applyTypeSubstitution: should not be here.");
    }
  }
  ...
  %strategy replaceFreshTypeVar(mnkt:ModularNewKernelTyper) extends Identity() {
    visit TomType {
      typeVar && Type(_,TypeVar(_)) << typeVar -> {
        if (mnkt.typeSubstitutions.containsKey(`typeVar)) {
          return mnkt.typeSubstitutions.get(`typeVar);
        }    
      }
    }
  }
