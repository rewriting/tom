...
public static final int TOM_CHANNEL = 1;
 ...
 *  (unseen by this grammar) that started an embedded Java statement
 *  block within Tom.
 */
RCURLY  : '}'
          {
          if ( nesting<=0 ) {
                ...
                System.out.println("exiting embedded java");
          }
          else {
                nesting--;
          }
...
TOM_MATCH : '%match'
          {
            // create a new tom lexer/parser duo that feeds
            // off the current input stream
            System.out.println("enter tom");
            TomLexer j = new TomLexer(input);
            CommonTokenStream tokens = new CommonTokenStream(j);
            tokens.discardTokenType(TomLexer.WS);
            Tom p = new Tom(tokens);
            p.javaMatchConstruct();
            // returns a TOM token to the java parser but on a
