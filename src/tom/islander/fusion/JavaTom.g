...
options {
  ...
  output=AST;
  ASTLabelType=Tree;
  tokenVocab=JavaTomTokens;
}

//tokens {
  //%include { javatom/JavaTomJavaTomTokenList.txt }
  //%include { minitomast/MiniTomMinitomAstTokenList.txt }
  //%include { minijavaast/MiniJavaMinijavaAstTokenList.txt }
//}
...
/**
 * MiniTom : the grammar has been written by following the 'official grammar' on the wiki
 */

// my temporary starting point 
tomCompilationUnit
  : blockList* EOF -> ^(TomCompilationUnit ^(BlockList blockList* ));
...
matchConstruct
  : MATCH LPAREN matchArguments RPAREN LBRACE patternAction* RBRACE -> ^(MatchConstruct matchArguments ^(PatternActionList patternAction* ))
  | MATCH LBRACE constraintAction* RBRACE -> ^(MatchConstructWithoutArgs ^(ConstraintActionList constraintAction* ))
  ;

/*
rule tata ta.ta.ta(ta ta) {
%match / %match(s)
ou rien, directement le srègles, (%match {} implicite)
}
*/

ruleConstruct
  : //'rule' t=Identifier fqn=Identifier LPAREN (pair (COMMA pair)*)? RPAREN LBRACE patternAction* RBRACE -> ^(RuleConstruct ^(Type $t ) ^(Name $fqn ) ^(TomTermList (pair+)? ) ^(PatternActionList patternAction* ))
//  'rule' Identifier FQN LPAREN /*(pair (COMMA pair)*)?*/ pairList RPAREN LBRACE constraintAction* RBRACE -> ^(RuleConstruct ^(Type Identifier ) ^(Name FQN ) pairList /*^(TomTermList (pair+)?*/ ^(ConstraintActionList constraintAction* ))

    //matchConstruct without the %match keyword
  'rule' Identifier FQN LPAREN pl=pairList? RPAREN LBRACE constraintAction* RBRACE
    -> {pl!=null}? ^(RuleConstruct ^(Type Identifier ) ^(Name FQN ) $pl /*pairList*/ ^(MatchConstructWithoutArgs ^(ConstraintActionList constraintAction* )))
    ->  ^(RuleConstruct ^(Type Identifier ) ^(Name FQN ) ^(EmptyList ) ^(MatchConstructWithoutArgs ^(ConstraintActionList constraintAction* )))

//  | 'rule' Identifier FQN LPAREN pl=pairList? RPAREN LBRACE matchConstruct RBRACE
//    -> {pl!=null}? ^(RuleConstruct ^(Type Identifier ) ^(Name FQN ) $pl /*pairList*/ matchConstruct )
//    ->  ^(RuleConstruct ^(Type Identifier ) ^(Name FQN ) ^(EmptyList ) matchConstruct )

  // this allows to have a rule construction with a constraint actions and a default return.
  | 'rule' (Identifier) FQN LPAREN pl=pairList? RPAREN LBRACE authorizedBlockList* RBRACE
    -> {pl!=null}? ^(RuleConstruct ^(Type Identifier ) ^(Name FQN ) $pl ^(BlockList authorizedBlockList* ) )
    ->  ^(RuleConstruct ^(Type Identifier ) ^(Name FQN ) ^(EmptyList ) ^(BlockList authorizedBlockList* ))

  ;

// what is authorized in a rule ?
authorizedBlockList
  : matchConstruct
  | javaCompilationUnit
  ;
...
/*  : cu=(matchConstruct* javaCompilationUnit (matchConstruct|javaCompilationUnit)*) -> ^(CompositeTerm ^(CompilationUnit $cu )) */
  : cu=compilationUnitList /*(ct=compilationUnit)**/ -> ^(CompositeTerm compilationUnitList /*^(BlockList ($ct)* )*/) /*(compilationUnit)* ))*/
  ;
...
//Include
includeConstruct
  : INCLUDE LBRACE (f=FILENAME | f=Identifier | f=FQN) RBRACE -> ^(Include $f ) 
  ;
...
//fragment
//CID: ( Identifier | DOLLARID | WS | STRING | DOT | LPAREN .* RPAREN | LBRACE .* RBRACE )+ ;
