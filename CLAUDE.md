...
correctes) et 4.F.16 (backquote application avec sous-terme dans le
body : `{ return \`Foo(x); }` → BQTermToInstruction(BQAppl(...,
concBQTerm(BQVariable(x))))) livrées.
...
**23 fixtures** sous `tomgo/testdata/parse/` valident byte-pour-byte
...
- `stable/tom/parser/equiv/` — harnais d'équivalence AST Go ⇄ Java :
  ...
  (`__INPUT__`/`__DIR__`).
- `testdata/parse/<name>/scenario.t` — 23 fixtures actuellement.
