...
*/
	...
	public final static void main( String[] args ) {
		PrimesT4 test = new PrimesT4( new PureFactory( 16 ));
		test.run(15);
	}
	...
	public PrimesT4( ATermFactory factory ) {
...
// ****** erreur ****** suc(y) n'est pas du bon type

		app(cons( x, suc(y) ), z)	-> cons( x, app( y, z ))
...
// ****** erreur ****** suc(y) n'est pas du bon type
			...
			cons( i, suc(y) ) -> { if ( ( toInt(t) % toInt(i) ) == 0 )
						{ return cons( i, y ); } } 
...
// ****** erreur ****** cons(x,y) n'est pas du bon type

			suc(suc(cons(x,y)))	-> { return cons( t, tnil ); }

// ****** erreur ****** cons(x,x) n'est pas du bon type

			suc(cons(x,x))	-> { return addifprime( suc(x), primes(x)); }
