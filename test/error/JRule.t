...
%rule {
  x@op1(_,_) -> x
  x@op1(_, op1(_,op1())) -> x // Bad number of arguments for symbol `op1`: 2 argument(s) required but 0 found (line 72)
  x@op1(_, op1(_,op3)) -> x // The symbol `op3` has type `type2` but type `type1` was required (line 73)
}

%rule {
  op3() -> op2(op3(), op3()) //The symbol `op2` has type `type1` but type `type2` was required (line 77)
