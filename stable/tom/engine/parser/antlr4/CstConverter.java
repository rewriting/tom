  ...
  private String tomSplitter(CstOptionList optionList,String subject, List<CstBlock> list) {
    ...
    for(int i=0 ; i<split.length ; i++) {
      if(metaMode) {
        ...
        list.add( tom.engine.adt.cst.types.cstblock.HOSTBLOCK.make(optionList, code) );
      } else {
        String code = "+"+split[i]+"+";
        //String code = split[i];
        metaMode = true;
    ...
    if(subject.endsWith(escapeChar)) {
      ...
      list.add( tom.engine.adt.cst.types.cstblock.HOSTBLOCK.make(optionList, "\"\"") );
    }
    return res;
  }
