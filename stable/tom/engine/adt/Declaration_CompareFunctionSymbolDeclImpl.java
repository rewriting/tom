...
extends Declaration
{
  ...
  {
    pattern = getStaticFactory().parse("CompareFunctionSymbolDecl(<term>,<term>,<term>,<term>)");
  }
  ...
  static public Declaration fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      Declaration tmp = getStaticTomSignatureFactory().makeDeclaration_CompareFunctionSymbolDecl(TomTerm.fromTerm( (aterm.ATerm) children.get(0)), TomTerm.fromTerm( (aterm.ATerm) children.get(1)), TargetLanguage.fromTerm( (aterm.ATerm) children.get(2)), Option.fromTerm( (aterm.ATerm) children.get(3)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
