...
public class TomTerm_BuildTermImpl
extends TomTerm
{
  ...
  public shared.SharedObject duplicate() {
    TomTerm_BuildTerm clone = new TomTerm_BuildTerm();
  ...
  {
    pattern = getStaticFactory().parse("BuildTerm(<term>,<term>)");
  }
  ...
  static public TomTerm fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      TomTerm tmp = getStaticTomSignatureFactory().makeTomTerm_BuildTerm(TomName.fromTerm( (aterm.ATerm) children.get(0)), TomList.fromTerm( (aterm.ATerm) children.get(1)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
