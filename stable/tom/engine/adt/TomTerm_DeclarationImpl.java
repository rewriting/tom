...
public class TomTerm_DeclarationImpl
extends TomTerm
{
  ...
  public shared.SharedObject duplicate() {
    TomTerm_Declaration clone = new TomTerm_Declaration();
  ...
  {
    pattern = getStaticFactory().parse("Declaration(<term>)");
  }
  ...
  static public TomTerm fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      TomTerm tmp = getStaticTomSignatureFactory().makeTomTerm_Declaration(TomTerm.fromTerm( (aterm.ATerm) children.get(0)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
