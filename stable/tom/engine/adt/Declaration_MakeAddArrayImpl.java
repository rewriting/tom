...
abstract public class Declaration_MakeAddArrayImpl
extends Declaration
{
  ...
  static public Declaration fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      Declaration tmp = getStaticTomSignatureFactory().makeDeclaration_MakeAddArray(TomName.fromTerm( (aterm.ATerm) children.get(0)), TomTerm.fromTerm( (aterm.ATerm) children.get(1)), TomTerm.fromTerm( (aterm.ATerm) children.get(2)), TargetLanguage.fromTerm( (aterm.ATerm) children.get(3)), Option.fromTerm( (aterm.ATerm) children.get(4)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
    ...
    switch(i) {
      ...
      case 3:
        if (! (arg instanceof TargetLanguage)) { 
          throw new RuntimeException("Argument 3 of a Declaration_MakeAddArray should have type TargetLanguage");
        }
        break;
      case 4:
        if (! (arg instanceof Option)) { 
          throw new RuntimeException("Argument 4 of a Declaration_MakeAddArray should have type Option");
        }
        break;
