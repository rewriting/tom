...
public class TomTerm_VariableStarImpl
extends TomTerm
{
  ...
  public shared.SharedObject duplicate() {
    TomTerm_VariableStar clone = new TomTerm_VariableStar();
  ...
  {
    pattern = getStaticFactory().parse("VariableStar(<term>,<term>,<term>)");
  }
  ...
  static public TomTerm fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      TomTerm tmp = getStaticTomSignatureFactory().makeTomTerm_VariableStar(Option.fromTerm( (aterm.ATerm) children.get(0)), TomName.fromTerm( (aterm.ATerm) children.get(1)), TomType.fromTerm( (aterm.ATerm) children.get(2)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
