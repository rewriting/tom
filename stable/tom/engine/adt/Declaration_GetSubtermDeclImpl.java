...
extends Declaration
{
  ...
  static public Declaration fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      Declaration tmp = getStaticTomSignatureFactory().makeDeclaration_GetSubtermDecl(TomTerm.fromTerm( (aterm.ATerm) children.get(0)), TomTerm.fromTerm( (aterm.ATerm) children.get(1)), TomTerm.fromTerm( (aterm.ATerm) children.get(2)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
  ...
  public aterm.ATermAppl setArgument(aterm.ATerm arg, int i) {
    switch(i) {
      ...
      case 2:
        if (! (arg instanceof TomTerm)) { 
          throw new RuntimeException("Argument 2 of a Declaration_GetSubtermDecl should have type TomTerm");
        }
        break;
