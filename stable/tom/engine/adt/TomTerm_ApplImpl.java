...
extends TomTerm
{
  ...
  public shared.SharedObject duplicate() {
    TomTerm_Appl clone = new TomTerm_Appl();
  ...
  {
    pattern = getStaticFactory().parse("Appl(<term>,<term>,<term>)");
  }
  ...
  static public TomTerm fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      TomTerm tmp = getStaticTomSignatureFactory().makeTomTerm_Appl(Option.fromTerm( (aterm.ATerm) children.get(0)), TomName.fromTerm( (aterm.ATerm) children.get(1)), TomList.fromTerm( (aterm.ATerm) children.get(2)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
