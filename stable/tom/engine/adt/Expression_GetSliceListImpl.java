...
extends Expression
{
  ...
  static public Expression fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      Expression tmp = getStaticTomSignatureFactory().makeExpression_GetSliceList(TomName.fromTerm( (aterm.ATerm) children.get(0)), TomTerm.fromTerm( (aterm.ATerm) children.get(1)), TomTerm.fromTerm( (aterm.ATerm) children.get(2)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
