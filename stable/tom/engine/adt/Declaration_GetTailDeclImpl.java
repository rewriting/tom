...
public class Declaration_GetTailDeclImpl
extends Declaration
{
  ...
  {
    pattern = getStaticFactory().parse("GetTailDecl(<term>,<term>)");
  }
  ...
  static public Declaration fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      Declaration tmp = getStaticTomSignatureFactory().makeDeclaration_GetTailDecl(TomTerm.fromTerm( (aterm.ATerm) children.get(0)), TargetLanguage.fromTerm( (aterm.ATerm) children.get(1)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
    ...
    switch(i) {
      case 0:
        if (! (arg instanceof TomTerm)) { 
          throw new RuntimeException("Argument 0 of a Declaration_GetTailDecl should have type TomTerm");
        }
        break;
      case 1:
        if (! (arg instanceof TargetLanguage)) { 
          throw new RuntimeException("Argument 1 of a Declaration_GetTailDecl should have type TargetLanguage");
        }
        break;
      default: throw new RuntimeException("Declaration_GetTailDecl does not have an argument at " + i );
    }
