...
extends TomTerm
{
  ...
  public shared.SharedObject duplicate() {
    TomTerm_MatchingCondition clone = new TomTerm_MatchingCondition();
  ...
  {
    pattern = getStaticFactory().parse("MatchingCondition(<term>,<term>)");
  }
  ...
  static public TomTerm fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      TomTerm tmp = getStaticTomSignatureFactory().makeTomTerm_MatchingCondition(TomTerm.fromTerm( (aterm.ATerm) children.get(0)), TomTerm.fromTerm( (aterm.ATerm) children.get(1)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
