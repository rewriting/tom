...
public class Declaration_GetFunctionSymbolDeclImpl
extends Declaration
{
  ...
  public shared.SharedObject duplicate() {
    Declaration_GetFunctionSymbolDecl clone = new Declaration_GetFunctionSymbolDecl();
  ...
  {
    pattern = getStaticFactory().parse("GetFunctionSymbolDecl(<term>,<term>,<term>)");
  }
  ...
  static public Declaration fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      Declaration tmp = getStaticTomSignatureFactory().makeDeclaration_GetFunctionSymbolDecl(TomTerm.fromTerm( (aterm.ATerm) children.get(0)), TargetLanguage.fromTerm( (aterm.ATerm) children.get(1)), Option.fromTerm( (aterm.ATerm) children.get(2)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
