...
extends Declaration
{
  ...
  {
    pattern = getStaticFactory().parse("IsEmptyDecl(<term>,<term>,<term>)");
  }
  ...
  static public Declaration fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      Declaration tmp = getStaticTomSignatureFactory().makeDeclaration_IsEmptyDecl(TomTerm.fromTerm( (aterm.ATerm) children.get(0)), TargetLanguage.fromTerm( (aterm.ATerm) children.get(1)), Option.fromTerm( (aterm.ATerm) children.get(2)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
