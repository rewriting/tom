...
public class TomTerm_RuleSetImpl
extends TomTerm
{
  ...
  {
    pattern = getStaticFactory().parse("RuleSet(<term>)");
  }
  ...
  static public TomTerm fromTerm(aterm.ATerm trm)
  {
    ...
    if (children != null) {
      TomTerm tmp = getStaticTomSignatureFactory().makeTomTerm_RuleSet(TomList.fromTerm( (aterm.ATerm) children.get(0)));
      tmp.setTerm(trm);
      return tmp;
    }
    else {
