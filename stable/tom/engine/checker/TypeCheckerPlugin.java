    ...
    if(isActivated()) {
      ...
      try {
        ...
        try {
          ...
          tom_make_TopDownCollect( new checkTypeInference(this) )
.visitLight(subject);
  ...
  private void verifyListVariableInAction(Instruction action) {
    try {
      tom_make_TopDown( new checkVariableStar(this) )
.visitLight(action);
        ...
        if(!tom___listName.equals(tom___rootName)) {
          TomMessage.error(tcp.getLogger(),
              ...
              TomMessage.incoherentVariableStar,
               tomMatch174_11.getString() 
, tom___rootName, tom___listName);
        }
