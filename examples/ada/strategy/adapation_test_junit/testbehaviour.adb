    ...
    function tom_make_Innermost(v: StrategyPtr) return StrategyPtr is 
    begin 
        return ( tom_make_Mu(newMuVar("_x"),tom_cons_list_Sequence(tom_make_All(newMuVar("_x")),tom_cons_list_Sequence(tom_make_Try(tom_cons_list_Sequence(v,tom_cons_list_Sequence(newMuVar("_x"),tom_empty_list_Sequence))),tom_empty_list_Sequence))) ); 
    end tom_make_Innermost; 

    function tom_make_Outermost(v: StrategyPtr) return StrategyPtr is 
    begin 
        return ( tom_make_Repeat(tom_make_OnceTopDown(v)) ); 
    end tom_make_Outermost; 
    ...
    type Builda is new AbstractStrategyBasic with null record;
        overriding function toString(t: Builda) return String;
        overriding function getChildren(v: access Builda) return ObjectPtrArrayPtr;
        overriding function setChildren(v: access Builda ; children: ObjectPtrArrayPtr) return VisitablePtr;
        overriding function getChildCount(v: access Builda) return Integer;
        overriding function getChildAt(v: access Builda; i : Integer) return VisitablePtr;
        overriding function setChildAt(v: access Builda; i: in Integer; child: in VisitablePtr) return VisitablePtr;
        overriding function visitLight(str:access Builda; v: ObjectPtr; intro: access Introspector'Class) return ObjectPtr;

    function newBuilda return StrategyPtr is
        newStrat : StrategyPtr := new Builda;
    begin
        makeAbstractStrategyBasic(AbstractStrategyBasic'Class(newStrat.all), tom_make_Identity);
        return newStrat;
    end newBuilda;

    function toString(t: Builda) return String is begin return "Builda()"; end;


    function  getChildren(v: access Builda) return ObjectPtrArrayPtr is
        stratChilds : ObjectPtrArrayPtr := new ObjectPtrArray(0..getChildCount(v));
    begin
        stratChilds(0) := ObjectPtr( getChildAt(AbstractStrategyBasic(v.all)'Access,0) );
        return stratChilds;
    ...
    function setChildren(v: access Builda ; children: ObjectPtrArrayPtr) return VisitablePtr is
    begin
        return setChildAt(AbstractStrategyBasic(v.all)'Access, 0, VisitablePtr(children(0)));
    end setChildren;

    function getChildCount(v: access Builda) return Integer is
    begin
        return 1;
    end getChildCount;

    function getChildAt(v: access Builda; i : Integer) return VisitablePtr is
        IndexOutOfBoundsException: exception;
    begin
        if i = 0 then
            return getChildAt(AbstractStrategyBasic(v.all)'Access, 0);
    ...
    function setChildAt(v: access Builda; i: in Integer; child: in VisitablePtr) return VisitablePtr is
        IndexOutOfBoundsException: exception;
    begin
        if i = 0 then
            return setChildAt(AbstractStrategyBasic(v.all)'Access, 0, child);
    ...
    function tom_visit_Term(str: access Builda; arg: TermPtr; intro: access Introspector'Class) return TermPtr is 
    begin 
        if not (( null  = str.env)) then 
            return TermPtr(visit(str.any, str.env, intro)); 
        else 
    ...
    function visit_Term(str: access Builda; tom_arg: TermPtr; intro: access Introspector'Class) return TermPtr is 
    begin 
        if tom_arg.all in Term'Class then 
    ...
    function visitLight(str: access Builda; v: ObjectPtr; intro: access Introspector'Class) return ObjectPtr is 
    begin 
        if v.all in Term'Class then 
            return ObjectPtr(visit_Term(str,TermPtr(v),intro)); 
        end if; 
    ...
    type Buildb is new AbstractStrategyBasic with null record;
        overriding function toString(t: Buildb) return String;
        overriding function getChildren(v: access Buildb) return ObjectPtrArrayPtr;
        overriding function setChildren(v: access Buildb ; children: ObjectPtrArrayPtr) return VisitablePtr;
        overriding function getChildCount(v: access Buildb) return Integer;
        overriding function getChildAt(v: access Buildb; i : Integer) return VisitablePtr;
        overriding function setChildAt(v: access Buildb; i: in Integer; child: in VisitablePtr) return VisitablePtr;
        overriding function visitLight(str:access Buildb; v: ObjectPtr; intro: access Introspector'Class) return ObjectPtr;

    function newBuildb return StrategyPtr is
        newStrat : StrategyPtr := new Buildb;
    begin
        makeAbstractStrategyBasic(AbstractStrategyBasic'Class(newStrat.all), tom_make_Identity);
        return newStrat;
    end newBuildb;

    function toString(t: Buildb) return String is begin return "Buildb()"; end;


    function  getChildren(v: access Buildb) return ObjectPtrArrayPtr is
        stratChilds : ObjectPtrArrayPtr := new ObjectPtrArray(0..getChildCount(v));
    begin
        stratChilds(0) := ObjectPtr( getChildAt(AbstractStrategyBasic(v.all)'Access,0) );
        return stratChilds;
    ...
    function setChildren(v: access Buildb ; children: ObjectPtrArrayPtr) return VisitablePtr is
    begin
        return setChildAt(AbstractStrategyBasic(v.all)'Access, 0, VisitablePtr(children(0)));
    end setChildren;

    function getChildCount(v: access Buildb) return Integer is
    begin
        return 1;
    end getChildCount;

    function getChildAt(v: access Buildb; i : Integer) return VisitablePtr is
        IndexOutOfBoundsException: exception;
    begin
        if i = 0 then
            return getChildAt(AbstractStrategyBasic(v.all)'Access, 0);
    ...
    function setChildAt(v: access Buildb; i: in Integer; child: in VisitablePtr) return VisitablePtr is
        IndexOutOfBoundsException: exception;
    begin
        if i = 0 then
            return setChildAt(AbstractStrategyBasic(v.all)'Access, 0, child);
    ...
    function tom_visit_Term(str: access Buildb; arg: TermPtr; intro: access Introspector'Class) return TermPtr is 
    begin 
        if not (( null  = str.env)) then 
            return TermPtr(visit(str.any, str.env, intro)); 
        else 
    ...
    function visit_Term(str: access Buildb; tom_arg: TermPtr; intro: access Introspector'Class) return TermPtr is 
    begin 
        if tom_arg.all in Term'Class then 
             return             tom_make_b            ;         end if; 
        return tom_visit_Term(str,tom_arg,intro); 
    end visit_Term; 

    function visitLight(str: access Buildb; v: ObjectPtr; intro: access Introspector'Class) return ObjectPtr is 
    begin 
        if v.all in Term'Class then 
            return ObjectPtr(visit_Term(str,TermPtr(v),intro)); 
        end if; 
    ...
    subject : TermPtr := tom_make_f(tom_make_a);
    s : StrategyPtr := tom_make_Identity;
    begin
    ...
    subject : TermPtr := tom_make_f(tom_make_a);
    s : StrategyPtr := tom_make_One(tom_make_R1);
    begin
    return  TermPtr( s.visitLight(VisitablePtr(subject)) );
    end;
    
    function test4 return TermPtr is
    subject : TermPtr := tom_make_g(tom_make_f(tom_make_a),tom_make_b);
    s : StrategyPtr := tom_make_OnceBottomUp((tom_make_R2));
    begin
    ...
    subject : TermPtr := tom_make_g(tom_make_f(tom_make_a),tom_make_b);
    s : StrategyPtr := tom_make_All((tom_make_R2));
    begin
    return  TermPtr( s.visitLight(VisitablePtr(subject)) );
    end;
    
    function testChoice return TermPtr is
    subject : TermPtr := tom_make_f(tom_make_b);
    s : StrategyPtr := tom_cons_list_Choice(tom_make_R2,tom_cons_list_Choice(tom_make_R1,tom_empty_list_Choice));
    begin
    return  TermPtr( s.visitLight(VisitablePtr(subject)) );
    end;
    ...
    subject : TermPtr := tom_make_f(tom_make_a);
    s : StrategyPtr := tom_cons_list_Choice(tom_cons_list_Sequence(tom_make_R1,tom_cons_list_Sequence(tom_make_R2,tom_empty_list_Sequence)),tom_cons_list_Choice(tom_make_R1,tom_empty_list_Choice));
    begin
    return  TermPtr( s.visitLight(VisitablePtr(subject)) );
    end;
    ...
    subject : TermPtr := tom_make_f(tom_make_b);
    s : StrategyPtr := tom_make_Not(tom_make_R2);
    begin
    return  TermPtr( s.visitLight(VisitablePtr(subject)) );
    end;
    ...
    subject : TermPtr := tom_make_f(tom_make_a);
    s : StrategyPtr := tom_make_Not(tom_cons_list_Sequence(tom_make_R1,tom_cons_list_Sequence(tom_make_R2,tom_empty_list_Sequence)));
    begin
    return  TermPtr( s.visitLight(VisitablePtr(subject)) );
    end;
    ...
    subject : TermPtr := tom_make_g(tom_make_f(tom_make_a),tom_make_b);
    s : StrategyPtr := tom_cons_list_Sequence(tom_make_All(tom_make_R2),tom_cons_list_Sequence(tom_make_All(tom_make_All(tom_make_R1)),tom_empty_list_Sequence));
    begin
    return  TermPtr( s.visitLight(VisitablePtr(subject)) );
    end;
    ...
    subject : TermPtr := tom_make_g(tom_make_f(tom_make_a),tom_make_b);
    s : StrategyPtr := tom_make_Omega(1,tom_make_R2);
    begin
    return  TermPtr( s.visitLight(VisitablePtr(subject)) );
    end;
    ...
    subject : TermPtr := tom_make_g(tom_make_f(tom_make_a),tom_make_b);
    s : StrategyPtr := tom_make_Omega(2,tom_make_R2);
    begin
    return  TermPtr( s.visitLight(VisitablePtr(subject)) );
    end;
    ...
    subject : TermPtr := tom_make_g(tom_make_f(tom_make_a),tom_make_b);
    s : StrategyPtr := tom_make_OnceTopDownId((tom_make_R1));
    begin
    subject :=  TermPtr( s.visitLight(VisitablePtr(subject)) );
    subject :=  TermPtr( s.visitLight(VisitablePtr(subject)) );
    return  TermPtr( s.visitLight(VisitablePtr(subject)) );
    ...
    subject : TermPtr := tom_make_g(tom_make_f(tom_make_a),tom_make_b);
    s : StrategyPtr := tom_make_Innermost((tom_make_R2));
    begin
    return  TermPtr( s.visitLight(VisitablePtr(subject)) );
    end;
    ...
    subject : TermPtr := tom_make_g(tom_make_f(tom_make_a),tom_make_b);
    s : StrategyPtr := tom_make_Outermost((tom_make_R2));
    begin
    return  TermPtr( s.visitLight(VisitablePtr(subject)) );
    end;
    ...
    --Test1
    assertTermEquals (tom_make_f(tom_make_a), test1 );
    assertTermEquals (tom_make_f(tom_make_a), test1_New );
    
    --Test2
    assertTermEquals ( tom_make_f(tom_make_b), test2 );
    assertTermEquals ( tom_make_f(tom_make_b), test2_New );
    
    --Test3
    assertTermEquals ( tom_make_f(tom_make_a), test3 );
    assertTermEquals ( tom_make_f(tom_make_a), test3_New );
    
    --Test4
    assertTermEquals ( tom_make_g(tom_make_f(tom_make_c),tom_make_c), test4 );
    assertTermEquals ( tom_make_g(tom_make_f(tom_make_c),tom_make_c), test4_New );
    
    --Test5  
    assertTermEquals ( tom_make_g(tom_make_f(tom_make_b),tom_make_c), test5 );
    assertTermEquals ( tom_make_g(tom_make_f(tom_make_b),tom_make_c), test5_New );
    
    --TestChoice
    assertTermEquals ( tom_make_f(tom_make_b), testChoice );
    assertTermEquals ( tom_make_f(tom_make_b), testChoice_New );
    
    --TestChoiceSideEffect
    assertTermEquals ( tom_make_f(tom_make_b), testChoiceSideEffect );
    assertTermEquals ( tom_make_f(tom_make_b), testChoiceSideEffect_New );
    
    --TestNot
    assertTermEquals ( tom_make_f(tom_make_b), testNot );
    assertTermEquals ( tom_make_f(tom_make_b), testNot_New );
    
    --TestNotSideEffect
    assertTermEquals ( tom_make_f(tom_make_a), testNotSideEffect );
    assertTermEquals ( tom_make_f(tom_make_a), testNotSideEffect_New );
    
    --Test6
    assertTermEquals (tom_make_g(tom_make_f(tom_make_c),tom_make_c), test6 );
    assertTermEquals (tom_make_g(tom_make_f(tom_make_c),tom_make_c), test6_New );
    
    --Test7
    assertTermEquals ( tom_make_g(tom_make_f(tom_make_b),tom_make_b), test7 );
    assertTermEquals ( tom_make_g(tom_make_f(tom_make_b),tom_make_b), test7_New );
    
    --Test8
    assertTermEquals ( tom_make_g(tom_make_f(tom_make_a),tom_make_c), test8 );
    assertTermEquals ( tom_make_g(tom_make_f(tom_make_a),tom_make_c), test8_New );
    
    
    --Test11  
    assertTermEquals ( tom_make_g(tom_make_f(tom_make_c),tom_make_c), test11 );
    assertTermEquals ( tom_make_g(tom_make_f(tom_make_c),tom_make_c), test11_New );
    
    --Test12
    assertTermEquals ( tom_make_g(tom_make_f(tom_make_c),tom_make_c), test12 ); 
    assertTermEquals ( tom_make_g(tom_make_f(tom_make_c),tom_make_c), test12_New ); 
    
    --Test13
    assertTermEquals ( tom_make_g(tom_make_f(tom_make_c),tom_make_c), test13 );
    assertTermEquals ( tom_make_g(tom_make_f(tom_make_c),tom_make_c), test13_New );
