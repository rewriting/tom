...
public class NshSharedSet {
  ...
  public NshSharedSet(TermFactory factory) {
    this.factory = factory;
    this.traversal = new GenericTraversal();
    this.result = new SharedSet(factory);
  }
  ...
  public boolean breadthSearch(State start, State end) {
    Collection c1 = new SharedSet(factory);
    c1.add(start);

    int i = 0;
    while(!c1.isEmpty()) {
      Collection c2 = new SharedSet(factory);
  ...
  public boolean depthSearch2(Collection local, State start, State end) {
    ...
    Iterator it = c1.iterator();
    Collection c = new SharedSet(factory);
    while(it.hasNext()) {
  ...
  public final static void main(String[] args) {
    NshSharedSet test = new NshSharedSet(new TermFactory(16));
    int nbAgent = 1;
    try {
      nbAgent = Integer.parseInt(args[0]);
      MaxMessagesInNetwork = Integer.parseInt(args[1]);
    } catch (Exception e) {
      System.out.println("Usage: java NshSharedSet <nbAgent> <maxMsg>");
      return;
    }
