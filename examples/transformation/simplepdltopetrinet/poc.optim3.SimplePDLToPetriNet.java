...
public class SimplePDLToPetriNet {
  ...
  private static  SimplePDLSemantics.DDMMSimplePDL.WorkSequenceType  tom_make_WorkSequenceTypestartToFinish() { 
  ...
  private static  SimplePDLSemantics.DDMMSimplePDL.WorkSequenceType  tom_get_default_WorkSequence_linkType() {
    return tom_make_WorkSequenceTypestartToStart()
      ;
  }
    ...
    public static void resolveInverseLinks(EObject resolveNode, EObject newNode, EObject acc) {
      ...
      for (EObject current : traced) {
        ...
        if (current instanceof  petrinetsemantics.DDMMPetriNet.PetriNet ) {
          ...
          } else { throw new RuntimeException("should not be there"); }} else if (current instanceof  petrinetsemantics.DDMMPetriNet.Place ) {
            petrinetsemantics.DDMMPetriNet.Place  newCurrent = ( petrinetsemantics.DDMMPetriNet.Place )current;
            if(newCurrent.getNet().equals(resolveNode) && toSet) {
              newCurrent.setNet((PetriNet)newNode); 
            } else { throw new RuntimeException("should not be there"); }} else if (current instanceof  petrinetsemantics.DDMMPetriNet.Arc ) {
              ...
              if(newCurrent.getTarget().equals(resolveNode) && toSet) {
                ...
                newCurrent.setNet((PetriNet)newNode); 
              } else { throw new RuntimeException("should not be there"); }} else if (current instanceof  petrinetsemantics.SDMMPetriNet.Place_dynamic ) {
                ...
                if(newCurrent.getNode_static().equals(resolveNode) && toSet) {
                ...
                } else { throw new RuntimeException("should not be there"); }} else if (current instanceof  petrinetsemantics.SDMMPetriNet.PetriNet_dynamic ) {
                  ...
                  if(newCurrent.getPetriNet_static().equals(resolveNode) && toSet) {
                  ...
                  } else { throw new RuntimeException("should not be there"); }} else if (current instanceof  petrinetsemantics.EDMMPetriNet.FireTransitionEvent ) {
                    ...
                    if(newCurrent.getFiredTransition().equals(resolveNode) && toSet) {
                    ...
                    } else { throw new RuntimeException("should not be there"); }} else if (current instanceof  petrinetsemantics.TM3PetriNet.PNScenario ) {
                      ...
                      } else { throw new RuntimeException("should not be there"); }} else if (current instanceof  petrinetsemantics.TM3PetriNet.PNSimEvent ) {
...
//      return tom_cons_list_Sequence(tom_make_TopDown(tom_make_P2PN(tom__linkClass,pn)),tom_cons_list_Sequence(tom_make_TopDown(tom_make_WD2PN(tom__linkClass,pn)),tom_cons_list_Sequence(tom_make_TopDown(tom_make_WS2PN(tom__linkClass,pn)),tom_empty_list_Sequence())));
    ...
    public static void main(String[] args) {
      ...
      String fileName="";
      System.out.println("\nStarting...\n");
      //processArgs(args);//might return a boolean
      ...
      if (args.length==1) {
        ...
        //strong hypothesis: 2 args+ is equivalent to $WD $HASWS (ok for the
        //moment)
        hasFileName=false;
        boolean hasWS = Boolean.parseBoolean(args[1]);
        System.out.println("Generation of a process composed of "+args[0]+" WD. ("+(hasWS?"with":"without")+" WS)");
        long startChrono = System.currentTimeMillis();
        p_root = GenModel.getModel(Integer.parseInt(args[0]),hasWS);
        long duration = System.currentTimeMillis()-startChrono;
        System.out.println("Generation done in "+duration+" (ms).\n");
        ...
        WorkSequence ws1 = 
          tom_make_WorkSequence(null,tom_make_WorkSequenceTypestartToStart(),wd1,wd2);
        WorkSequence ws2 = 
          tom_make_WorkSequence(null,tom_make_WorkSequenceTypestartToFinish(),wd3,wd4);
...
resolve(translator);
        ...
        if(hasFileName) {
          prefix = prefix+"_"+fileName.substring(0, fileName.lastIndexOf('.'));
        }
        ...
        tom_cons_list_Sequence(tom_make_TopDown(tom_make_PrintTransition()),tom_cons_list_Sequence(tom_make_TopDown(tom_make_PrintPlace()),tom_empty_list_Sequence())).visit(translator.pn,
...
//options
   ...
   * aaaa
   * 
   * @param args the argmuments list
   */
  public static void processArgs(String[] args) {
    // -gm: generate input model
    // -sr: serialize output model
    // -in: give input model
    // -xmi: serialize generated model
    int i = 0;
    String argument = "";
    try {
      for(; i < args.length; i++) {
        ...
        if(!argument.startsWith("-") || (argument.equals("-"))) {
          ...
          if(argument.equals("gm")) {
            //TODO
            //genModel=true;
            //inputFile=null;
          }
          if(argument.equals("xmi")) {
            //TODO
            //gen2xmi = true;
            //inputModelFile=;
          }
          if(argument.equals("in")) {
            //TODO
            //inputFile=(arg[i+1].startsWith("-")?"models/DefaultSimplePDLInstance.xmi":args[++i])
          }
          if(argument.equals("sr")) {
            //TODO
            //serializeResult=true;
          }
          if(argument.equals("bench")) {
            //TODO
            //tout à false, sauf
            //benchMode=true;
            //genModel=false;
  ...
  private static void displayHelp() {
    System.out.println("usage: java -cp $YOUR_CLASSPATH [options]");
    System.out.println("options:");
        System.out.println("  -gm [<WD number> <hasResolve> [-xmi [<file name>]]] (generates a model following the #1 example ; two optional parameters: the number of WD and the fact that the generated model will lead to generation of resolve nodes)");
        System.out.println("  -xmi [<file name>] (serializes)");
        System.out.println("  -in [<file>] (gives a .xmi model as input, when omitted the default file is models/DefaultSimplePDLInstance.xmi");
        System.out.println("  -sr (serializes results into .xmi and .net files)");
        System.out.println(" ");
  }
