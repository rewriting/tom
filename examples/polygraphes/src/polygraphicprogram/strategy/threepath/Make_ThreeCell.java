...
public class Make_ThreeCell implements tom.library.sl.Strategy {
  ...
  public Make_ThreeCell(String _Name, tom.library.sl.Strategy _Source, tom.library.sl.Strategy _Target, tom.library.sl.Strategy _Type) {
    this._Name = _Name;
  ...
  /**
    * Builds a new ThreeCell
    * If one of the sub-strategies application fails, throw a VisitFailure
    */
  public tom.library.sl.Visitable visitLight(tom.library.sl.Visitable any) throws tom.library.sl.VisitFailure {

    polygraphicprogram.types.TwoPath new_Source = (polygraphicprogram.types.TwoPath) _Source.visit(any);

    polygraphicprogram.types.TwoPath new_Target = (polygraphicprogram.types.TwoPath) _Target.visit(any);

    polygraphicprogram.types.CellType new_Type = (polygraphicprogram.types.CellType) _Type.visit(any);

    return polygraphicprogram.types.threepath.ThreeCell.make( _Name,  new_Source,  new_Target,  new_Type);
  }

  public int visit() {

    (_Source).visit();
    polygraphicprogram.types.TwoPath new_Source = (polygraphicprogram.types.TwoPath) getEnvironment().getSubject();

    (_Target).visit();
    polygraphicprogram.types.TwoPath new_Target = (polygraphicprogram.types.TwoPath) getEnvironment().getSubject();

    (_Type).visit();
    polygraphicprogram.types.CellType new_Type = (polygraphicprogram.types.CellType) getEnvironment().getSubject();

    getEnvironment().setSubject(polygraphicprogram.types.threepath.ThreeCell.make( _Name,  new_Source,  new_Target,  new_Type));
