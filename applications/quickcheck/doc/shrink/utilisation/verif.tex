...
\noind Pour formaliser la syntaxe, on se munit d'un ensemble infini $\X$ de symboles de \textit{variables} et un ensemble $\sig$ de symboles de fonctions. Chaque fonction a une \textit{arité}. On peut alors définir la notion de termes de manière inductive :
\begin{itemize}
\item toute variable est un terme,
\item tout symbole de fonction d'arité 0 est un terme,
...
\noind On peut interpréter une formule syntaxique issue d'une signature $\sig$, de prédicats $\predicat$ et de domaines $\D$ en lui associant une \textit{interprétation} $I$ qui est la connaissance d'un couple $(\d_I, \ent{\cdot}_I)$ où
\begin{itemize}
...
\item Si $s$ est un symbole de fonction d'arité $n$ et $t_1, t_2, \dots, t_n$ sont des termes, 
...
\item les symboles de fonctions de l'ensemble $\sig$ correspondent à des objets implémentant l'interface \texttt{FunctionInterpretation}. Chacune de ces interprétations doit fournir une méthode \texttt{compute} qui à une liste de termes associe la valeur souhaitée de l'interprétation du symbole dans ce cas.
