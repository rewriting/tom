...
public class Compiler {
	...
	public static void main (String args[]) {
		if(args.length!=2){
			...
			if(args[0].substring(args[0].length()-4, args[0].length()).equals(".xml")){
				programPath=args[0];
				System.out.println("DEBUG programPath : " + programPath);
				System.out.println("DEBUG File sperator : " + File.separator);
				if(args[1].substring(args[1].length()-1, args[1].length()).equals(File.separator)){//"\")){ code de linux
					outputFolderPath=args[1];
					System.out.println("DEBUG outputFolderPath : " + outputFolderPath);
					...
					try{
						...
						boolean res = f.mkdir();
						System.out.println("Folder " + outputFolderPath + " created : " + res);
						f = new File(outputFolderPath + "program" + File.separator);
						res = f.mkdir();
						System.out.println("Folder program created : " + res);
		     			File programFile = new File(outputFolderPath+ "program" + File.separator + programName + ".t");
						if(!programFile.exists()){
							System.out.println("DEBUG : File " +  programName + ".t" +" created");
							programFile.createNewFile();
						} else {
							System.out.println("DEBUG : File " +  programName + ".t" +" already exists");
						}
						...
						System.out.println("DEBUG : path = " + srcpath);
						File gomFile=new File(outputFolderPath + "adt" + File.separator + programName + ".gom");
						res = (new File(outputFolderPath+"adt" + File.separator)).mkdir();
						System.out.println("Folder " + outputFolderPath +"adt" + File.separator + " created : " + res);
						if(!gomFile.exists()){
							System.out.println("DEBUG : File " + outputFolderPath + "adt" + File.separator + programName + ".gom" +" created");
							gomFile.createNewFile();
						} else {
							System.out.println("DEBUG : File " + outputFolderPath + "adt" + File.separator + programName + ".gom" +" already exists");
						}
						String gomFileContent = XMLhandler.load(srcpath+".." + File.separator + "src" + File.separator + "adt" + File.separator + "PolygraphicProgram.gom");
						...
						File xmlHandlerFile=new File(outputFolderPath + "library"+ File.separator + programName + "Tools.t");
						res = (new File(outputFolderPath+"library" + File.separator)).mkdir();
						System.out.println("Folder " + outputFolderPath +"library" + File.separator + " created : " + res);
						if(!xmlHandlerFile.exists()){
							System.out.println("DEBUG : File " + outputFolderPath + "library"+ File.separator + programName + "Tools.t" +" created");
							xmlHandlerFile.createNewFile();
						} else {
							System.out.println("DEBUG : File " + outputFolderPath + "library"+ File.separator + programName + "Tools.t" +" already exists");
						}
						String XmlhandlerFileContent=XMLhandler.load(srcpath + ".." + File.separator + "src" + File.separator + "library" + File.separator +"XMLhandler.t");
						//System.out.println("DEBUG: " + XmlhandlerFileContent);
						...
						//Attention chemins linux dans les appels includes de tom
						...
						XMLhandler.save(XmlhandlerFileContent,xmlHandlerFile);
						//create script file
						File scriptFile;
						if (getOS() == OS_WIN) {
						...
						XMLhandler.save(makeScript(programName),scriptFile);
					}			
	...
	public static String makeScript(String programName){
		...
		return %[@entete@
@appelcall@gom adt@mpath@.gom
@appelcall@tom library@mpath@Tools.t 
@appelcall@tom program@mpath@.t
javac library@mpath@Tools.java
javac program@mpath@.java
]%;
		}
	...
	public static String makeTomFile(String filePath){
		...
		//Attention chemins linux dans les appels includes de tom
		String tomPath="../adt/" +lowerProgramName + "/" + programName+".tom";//make tom path
