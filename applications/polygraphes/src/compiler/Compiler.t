...
public class Compiler {
			...
			if(args[0].substring(args[0].length()-4, args[0].length()).equals(".xml")){
				...
				if(args[1].substring(args[1].length()-1, args[1].length()).equals("/")){
					...
					try{
						...
						(new File(outputFolderPath+"program/")).mkdir();
						File programFile=new File(outputFolderPath+"program/"+programName+".t");
	...
	public static String makeScript(String programName){
		return %[#!/bin/tcsh
gom adt/@programName@.gom
tom library/@programName@Tools.t 
tom program/@programName@.t
javac library/@programName@Tools.java
javac program/@programName@.java
]%;
		}
