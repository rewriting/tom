...
sort TwoPath:block(){
	...
	public  String prettyPrintBis(){
		%match (this){
						t@@TwoCell(_,_,_,_,_) -> { return `t.getName()+"["+`t.getID()+"]"; }
...
%match (TwoPath path){
			...
			TwoCell(name,source,target,type,_) -> { return "<TwoCell Name=\""+`name+"\" Type=\""+`type.toString().replace("()","")+"\">\n<Source>\n"+onePath2XML(`source)+"</Source>\n<Target>\n"+onePath2XML(`target)+"</Target>\n</TwoCell>\n"; }
