...
public class ATerm2AntlrOptional {
    ...
    public static AntlrElement getAntlrOptional(ATerm t) throws AntlrWrongElementException {
        %match(t) {
            OPTIONAL(_,l) -> {
                Container container=new Container();
                parseArgs(`l,container);
                if(container.goodParse) {
                    return `AntlrOptional(container.element);
                } else {
                    if(container.wrong!=null) {
                        throw new AntlrWrongElementException(
                            `AntlrIncorrectOptional(container.element,container.wrong));
                    } else {
                        throw new AntlrWrongElementException(
                            `AntlrIncorrectOptionalArg(container.element));
                    }
                }
            }
        }
        
        throw new AntlrWrongElementException(`AntlrPlainWrongOptional(ATerm2AntlrWrong.getAntlrWrong(t)));
    }
