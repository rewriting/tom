...
security analysis in a declarative way using the {\tom} language.
...
As an example, suppose we want to create a tool that replaces the use of text
literals with calls to \verb+getProperty("...")+ in order to facilitate
internationalization. For example, suppose we have the following code:
...
\begin{tomcode} 
  ...
  public String replaceLiteral(String expression) {
...
Another example consists in identifying all method calls that do not start
with 'get' - these are suspects for code that mutates the state of the
application during rendering of the page.
