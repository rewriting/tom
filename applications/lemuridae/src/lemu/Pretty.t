...
public class Pretty {
  ...
  private static String pr2(String var1, RawProp prop1, String var2, RawProp prop2) {
    return churchStyle ? %[<@var1@:@`pretty(prop1)@,@var2@:@pretty(prop2)@>]% : %[<@var1@,@var2@>]%;
  }
  ...
  public static String pretty(RawProofTerm t) {
    %match(t) {
      ...
      RawandL(RawAndLPrem1(x,px,y,py,M),n) -> { return %[andL(@`pr2(x,px,y,py)@ @`pretty(M)@,@`n@)]%; }
      RaworR(RawOrRPrem1(a,pa,b,pb,M),cn) -> { return %[orR(<@`pr2(a,pa,b,pb)@ @`pretty(M)@,@`cn@)]%; }
      RaworL(RawOrLPrem1(x,px,M),RawOrLPrem2(y,py,N),n) -> { return %[orL(@`pr(x,px)@ @`pretty(M)@,@`pr(y,py)@ @`pretty(N)@,@`n@)]%; }
      RawimplyR(RawImplyRPrem1(x,px,a,pa,M),cn) -> { return %[implyR(@`pr2(x,px,a,pa)@ @`pretty(M)@,@`cn@)]%; }
