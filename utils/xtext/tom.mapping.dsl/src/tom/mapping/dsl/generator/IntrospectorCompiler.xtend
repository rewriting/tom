...
class IntrospectorCompiler {
	...
	def compile(Mapping map, IFileSystemAccess fsa) {
		fsa.generateFile(prefix+"/"+map.name.toFirstLower()+"/internal"+map.name.toFirstUpper()+"Instrospector.java", map.introspector());
	}
	...
	def getChildren(Mapping map) {
		val packages = getAllPackages(map);
		...
		public Object[] getChildren(Object arg0) {
			...
			if(arg0 instanceof List) {
				// Children of a lis are its content
		...
		private static IChildrenGetter old_getter(Object o) {
			...
			«for(package: packages)» {
				if(eo.eClass().eContainer() == «package.name.toFirstUpper()»Package.eINSTANCE) {
					return «getChildrenGetterNamer(package)».INSTANCE;
				}
			}
		...
		«for(package : packages)» {«injchi.getter(map,package);»}
		'''
	}
	...
	def setChildren(Mapping map) {
		...
		«for(package: packages) {injchi.setter(map, package),}»
		'''
	}
