...
class TomFactoryCompiler {
	 ...
	 */
	 ...
	 public class «tomFactoryName(map)» {
	 	
	 	/* PROTECTED REGION ID(map.name+"_tom_factory_instances") ENABLED START */
	 	
	 	«val packageList = map.operators.filter[j | j instanceof ClassOperator].collect(e | e.class_.EPackage);»										// What is collect ?
	 	«for(package: packageList.filter[!e | packageList.filter[f | e.name == f.name && e!=f]])» {													// Ok for "reject" ?
	 		public static «package.name.toFirstUpper()»Factory «package.name»Factory = «package.name.toFirstUpper)»Factory.eINSTANCE;
	 	}
	 	
	 	«val packageList = map.allDefaultOperators.collect(e | e.EPackage);»																			// What is collect ?
	 	«for(package: packageList.filter[!e | packageList.filter[f | e.name == f.name && e!=f]])» {													// Ok for "reject" ?
	 		public static «package.name.toFirstUpper()»Factory «package.name»Factory = «package.name.toFirstUpper)»Factory.eINSTANCE;
	 	}
	 }
	...
	def operator(Mapping mapping, ClassOperator clop) {
		if(clop.parameters.size>0) {
			...
			javaFactoryCreateOperatorWithParameters(parameters, mapping, clop);											// Encore le probl�me List[] ou List<>. Cast sans doute provisoire
		} else {
	...
	public static «clop.class_.name» create«clop.name.toFirstUpper()»(«for(p: parameters SEPARATOR ","» {«typeOfParameter(mapping, p.feature)» _«p.feature.name»}) {
	...
	def javaFactoryCreateDefaultOperator(Mapping mapping, String name, EClass ecl) {
		...
		«for(p: parameters SEPARATOR ",")»{«typeOfParameter(mapping, p)» _«p.name»}) {
	...
	def dispatch terminalTypeName(Mapping mapping, boolean many, EClass ecl) {
		if(many) {
			'''
			List<''' } '''«mapping.getTerminal(ecl,false).class_.name»''' 
			if(many) {
			'''>'''}
	}
	...
	def javaClassAttributes(Mapping mapping, EClass ecl) {
		...
		«if(ecl.EAllAttribute.size > 0 && ecl.getDefaultParameters(mapping).size > 0)» {,}
		'''
	}
