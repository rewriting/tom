...
class OperatorsCompiler {
	...
	«IF mapping.getTerminal(ecl,false) != null»
		...
		make(«FOR att: ecl.EAllAttributes SEPARATOR ","» «att.name»«ENDFOR»«IF (ecl.EAllAttributes.size > 0) && (ecl.getDefaultParameters(mapping).size > 0)»,«ENDIF»«FOR param: parameters SEPARATOR ","»«param.name»«ENDFOR»){«mapping.tomFactoryQualifiedName()».
«op.toFirstUpper()»(«FOR att: ecl.EAllAttributes SEPARATOR ","»$«att.name»«ENDFOR»«IF ecl.EAllAttributes.size > 0 && ecl.getDefaultParameters(mapping).size > 0»,«ENDIF»«FOR param: parameters SEPARATOR ","» $«param.name»«ENDFOR»)}«ENDIF»}
   	'''
	}
   ...
   «val parameters = clop.getCustomParameters()»
		...
		%op «mapping.getTerminal(clop.class_,false).name» «clop.name»(«FOR p : parameters SEPARATOR ","»«injpa.featureParameter(mapping, p)»«ENDFOR») {
	...
	def getter(EClass c, EStructuralFeature esf) {
		'''
		«IF esf.many»enforce«ENDIF»((«c.name»)$t).get«esf.name.toFirstUpper()»()«IF esf.many»)«ENDIF»
		'''
	}
	...
	def classAttributes(Mapping mapping, EClass ecl) {		
		'''		
		«FOR att: ecl.EAllAttributes SEPARATOR ","»«att.name» : «injpa.feature(att)»«ENDFOR»«IF (ecl.EAllAttributes.size() > 0) && (ecl.getDefaultParameters(mapping).size() >0)»,«ENDIF»
		'''
	}
	...
	def javaClassAttributes(Mapping mapping, EClass ecl) {
		'''		
		«FOR att: ecl.EAllAttributes SEPARATOR ","»«injpa.feature(att)» _«att.name»«ENDFOR»«IF (ecl.EAllAttributes.size > 0) && (ecl.getDefaultParameters(mapping).size>0)»,«ENDIF»
		'''
	}
