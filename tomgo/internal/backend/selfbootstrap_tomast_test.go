...
func TestSelfBootstrap_Tomast(t *testing.T) {
	...
	adtDir := filepath.Join(repo, "src", "tom", "engine", "adt")
	committedDir := filepath.Join(repo, "tomgo", "internal", "tomast")
	...
	for _, name := range expectedGoNames {
		...
		if !bytes.Equal(regen, committed) {
			t.Errorf("%s: regenerated content differs from committed internal/tomast/%s — "+
